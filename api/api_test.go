...
func testCoinInfo(t *testing.T, server *httptest.Server) {
	tests := []struct {
		...
		},
		{
			...
			requestBody:    "",
			expectedStatus: 200,
			expectedBody:   "{\"circulating_supply\": 0, \"assets\": {}, \"market_cap\": 0, \"total_supply\": 0, \"volume_24\": 0}",
		},
		{
			name:           "test coin assets not found",
			requestUrl:     fmt.Sprintf("%s/v1/market/assets?coin=500&token=ETHToken", server.URL),
			requestMethod:  "GET",
			requestBody:    "",
			expectedStatus: 200,
			expectedBody:   "{\"circulating_supply\": 0, \"assets\": {}, \"market_cap\": 0, \"total_supply\": 0, \"volume_24\": 0}",
